	...
	public JF_AñadirCliente(Clientes_Admin frame,String DNI,String tabla) {										//Para modificar o eliminar
		...
		try {
			...
			if(tabla.equals("Usuario")) {
				...
				campo1 = result.getString("dni_"+tabla.toLowerCase());
				System.out.println("contraseña_"+tabla.toLowerCase());
				campo2 = result.getString("contraseña_"+tabla.toLowerCase());
				campo3 = result.getString("nombre_"+tabla.toLowerCase());
				campo4 = result.getString("correo_electronico_"+tabla.toLowerCase());
				campo5 = result.getString("telefono_"+tabla.toLowerCase());
				campo6 = result.getString("especialidad_"+tabla.toLowerCase());
				campo7 = result.getString("estado_alta_"+tabla.toLowerCase());
			}
		...
		EliminarCliente.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(success) {
					login.conexion.DML("DELETE From derrap."+tabla+" where derrap."+tabla+".dni_"+tabla+" = "+"'"+info.get(0).getText()+"'");
					System.out.println("DELETE From derrap."+tabla+" where dni_"+tabla+" = "+"'"+info.get(0).getText()+"'");
		...
		Guardar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {   //Codigo modficiar Update
				boolean success = true;
				int index = 0;
				...
				if(success) {
					...
					if(tabla.equals("Cliente")) {
						...
						login.conexion.DML("UPDATE "+tabla+" SET contraseña_usuario = '"+datos[1]+"', nombre_usuario = '"+datos[2]+"' , correo_electronico_usuario = '"+datos[3]+"', telefono_usuario = '"+datos[4]+
								"', especialidad_usuario = '"+datos[5]+"',estado_alta_usuario = '"+datos[6]+"' WHERE dni_usuario = '"+datos[0]+"'");
					}
					dispose();
					frame.ActualizarTabla(tabla);
				}
