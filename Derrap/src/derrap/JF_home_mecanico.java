...
public class JF_home_mecanico extends JFrame {
	...
	public JF_home_mecanico() {
		// login.conexion.consulta(, getWarningString(), getName()) /////Revisar esta
		// linea
		// Creacion ordenes

		ArrayList<Orden> ordenes_usuario = new ArrayList<Orden>();
		for (int i = 1; i <= login.conexion
				.consulta_Numero_Registros("SELECT COUNT(*) from orden_trabajo WHERE dni_usuario_orden_trabajo='"
						+ login.dniusuario + "' AND id_estado_reparacion_orden_trabajo!=4;"); i++) {
			if (login.conexion.consultaCampo("id_orden_trabajo", "orden_trabajo", "WHERE dni_usuario_orden_trabajo='"
					+ login.dniusuario + "' AND id_estado_reparacion_orden_trabajo!=4") != "") {
				int id = Integer.parseInt(login.conexion.consultaCampo("id_orden_trabajo", "orden_trabajo",
						"WHERE dni_usuario_orden_trabajo='" + login.dniusuario
								+ "' AND id_estado_reparacion_orden_trabajo!=4"));
		...
		if (ordenes_usuario.size() > 0) {
			...
			JLabel lbTítuloOrden1 = new JLabel(ordenes_usuario.get(0).getMarca_coche() + " "
					+ ordenes_usuario.get(0).getModelo_coche() + ", " + ordenes_usuario.get(0).getMatricula());
			...
			JLabel lbIngresoTallerOden1 = new JLabel(
					"Fecha de ingreso al taller: " + ordenes_usuario.get(0).getFecha_entrada());
			...
			JLabel lblEstadoOrden1 = new JLabel(
					"Estado de la reparación: " + ordenes_usuario.get(0).getEstado_reparacion());
			...
			// este label es el que contendrá la imagen del icono. Lo coloco en el centro
			// del borderlayout
			...
			contentPane.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(ComponentEvent e) {
					...
					Image imagenRedimensionadaMatricula = imagenMatricula.getScaledInstance(width, height,
							Image.SCALE_SMOOTH);
			...
			contentPane.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(ComponentEvent e) {
					...
					Image imagenRedimensionadaServicios = imagenServicios.getScaledInstance(width, height,
							Image.SCALE_SMOOTH);
			...
			contentPane.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(ComponentEvent e) {
					...
					Image imagenRedimensionadaCalendario = imagenCalendario.getScaledInstance(width, height,
							Image.SCALE_SMOOTH);
			...
			contentPane.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(ComponentEvent e) {
					...
					Image imagenRedimensionadaDescripcion = imagenDescripcion.getScaledInstance(width, height,
							Image.SCALE_SMOOTH);
			...
			contentPane.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(ComponentEvent e) {
					...
					Image imagenRedimensionadaEstado = imagenEstado.getScaledInstance(width, height,
							Image.SCALE_SMOOTH);
		...
		if (ordenes_usuario.size() > 1) {
			...
			JLabel lbTítuloOrden2 = new JLabel(ordenes_usuario.get(1).getMarca_coche() + " "
					+ ordenes_usuario.get(1).getModelo_coche() + ", " + ordenes_usuario.get(1).getMatricula());
			...
			JLabel lbIngresoTallerOden2 = new JLabel(
					"Fecha de ingreso al taller: " + ordenes_usuario.get(1).getFecha_entrada());
			...
			JLabel lblEstadoOrden2 = new JLabel(
					"Estado de la reparación: " + ordenes_usuario.get(1).getEstado_reparacion());
		...
		if (ordenes_usuario.size() > 2) {
			...
			JLabel lbTítuloOrden3 = new JLabel(ordenes_usuario.get(2).getMarca_coche() + " "
					+ ordenes_usuario.get(2).getModelo_coche() + ", " + ordenes_usuario.get(2).getMatricula());
			...
			JLabel lbIngresoTallerOden3 = new JLabel(
					"Fecha de ingreso al taller: " + ordenes_usuario.get(2).getFecha_entrada());
			...
			JLabel lblEstadoOrden3 = new JLabel(
					"Estado de la reparación: " + ordenes_usuario.get(2).getEstado_reparacion());
		 ...
		 * menuLateral = new JPanel(); menuLateral.setBackground(Color.GRAY);
		 * menuLateral.setBounds(0, 0, 200, 949); // Ajusta el tamaño del menú
		 * menuLateral.setLayout(new BoxLayout(menuLateral, BoxLayout.Y_AXIS)); //
		 * Layout vertical contentPane.add(menuLateral);
		 * 
		 * // Botones del menú lateral JButton btnInicio = new JButton("Inicio");
		 * JButton btnConfiguracion = new JButton("Configuración"); JButton btnSalir =
		 * new JButton("Salir");
		 * 
		 ...
		 * 
		 * // Acción para el botón "Inicio" btnInicio.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * JOptionPane.showMessageDialog(JF_home_mecanico.this,
		 * "Botón 'Inicio' presionado"); } });
		 * 
		 * // Acción para el botón "Configuración"
		 * btnConfiguracion.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) {
		 * JOptionPane.showMessageDialog(JF_home_mecanico.this,
		 * "Botón 'Configuración' presionado"); } });
		 * 
		 * // Acción para el botón "Salir" btnSalir.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * System.exit(0); // Cerrar la aplicación } });
		 * 
		 * // Agregar los botones al menú lateral menuLateral.add(btnInicio);
		 * menuLateral.add(btnConfiguracion); menuLateral.add(btnSalir);
		 * 
		 * // Panel central (contenido principal) JPanel panelContenido = new JPanel();
		 * panelContenido.setBounds(200, 0, 801, 949); // Establecer área para el
		 * contenido principal panelContenido.setBackground(Color.WHITE);
		 * contentPane.add(panelContenido);
		 * 
		 * // Puedes agregar más elementos al panel central, como etiquetas, campos de
		 * texto, etc.
		 * 
		 */
	}
