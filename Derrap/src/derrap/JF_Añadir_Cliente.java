...
public class JF_Añadir_Cliente extends JFrame {
	...
	public JF_Añadir_Cliente(JF_Cliente_Admin frame, String DNI, String tabla) { // Para modificar o eliminar
		...
		if (!tabla.equals("Vehiculo")) {
			result = login.conexion
					.consulta("Select * From " + tabla + " WHERE dni_" + tabla + " = " + "'" + DNI + "'");
		} else {
			result = login.conexion
					.consulta("Select * From " + tabla + " WHERE matricula_" + tabla + " = " + "'" + DNI + "'");
		}
					...
					if (success) {
						if (!tipoUsuario.equals("Vehiculo")) {
							...
							login.conexion.DML("DELETE From derrap." + tabla + " where derrap." + tabla + ".dni_"
									+ tabla + " = " + "'" + info.get(0).getText() + "'");// Hay que mirar que los
																							// usuarios tienen coches
																							// asociados y por eso no se
																							// pueden borrar
						} else {
							...
							// login.conexion.DML("DELETE From derrap.factura WHERE id_orden_trabajo ="+"'"+
							// id_orden+"'");
							login.conexion.DML("DELETE From derrap." + tabla + " where derrap." + tabla + ".matricula_"
									+ tabla + " = " + "'" + info.get(0).getText() + "'");
						}
	...
	public JF_Añadir_Cliente() { // Para poder ver el frame en Design
		...
		AñadirCliente.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				System.out.println(success);
				if (success) {
					login.conexion.DML("INSERT INTO derrap.cliente VALUES('" + datos[0] + "', '" + datos[1] + "' , '"
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (success) {
					if (tipoUsuario.equals("Cliente")) {
						...
						System.out.println(
								"INSERT INTO derrap." + tabla + " VALUES('" + datos[0] + "', '" + datos[1] + "' , '"
										+ datos[2] + "', '" + datos[3] + "' , '" + datos[5] + "', '" + datos[4] + "')");
						login.conexion.DML("INSERT INTO derrap." + tabla + " VALUES('" + datos[0] + "', '" + datos[1]
								+ "' , '" + datos[2] + "', " + datos[3] + " , '" + datos[5] + "', '" + datos[4] + "')");
					}
					frame.ActualizarTabla(tabla);
