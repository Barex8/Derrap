...
public class JF_Añadir_Orden extends JFrame {
	...
	public JF_Añadir_Orden(JF_Ordenes_Admin frame, String ID) {			 // Para modificar o eliminar
		...
		String tabla = "id_orden_trabajo";
		ResultSet result = null;
		result = login.conexion.consulta(
				"Select * From Derrap." + tabla + " WHERE id_" + tabla + " = " + "'" + ID + "'");
		System.out
				.println("Select * From Derrap." + tabla + " WHERE id_" + tabla + " = " + "'" + ID + "'");
		try {
			result.next();
			campo1 = result.getString("fecha_entrada_" + tabla.toLowerCase());
			campo2 = result.getString("fecha_salida_" + tabla.toLowerCase());
			campo3 = result.getString("descripcion_" + tabla.toLowerCase());
			campo4 = result.getString("sustitucion_pieza_" + tabla.toLowerCase());
