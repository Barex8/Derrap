...
public class JF_ordenes extends JFrame {
	public JF_ordenes() {
        ...
        for (int i = 1; i <= login.conexion.consulta_Numero_Registros("SELECT COUNT(*) from orden_trabajo WHERE id_estado_asignacion_orden_trabajo=1 AND id_estado_reparacion_orden_trabajo!=4;"); i++) {
        	String num_id = login.conexion.consultaCampo("id_orden_trabajo", "orden_trabajo", "WHERE id_estado_asignacion_orden_trabajo=1 AND id_estado_reparacion_orden_trabajo!=4;");
